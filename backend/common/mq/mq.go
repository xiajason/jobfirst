...
func (m *MQManager) publishRedis(ctx context.Context, message *Message) error {
	// 这里应该调用Redis发布消息
	// 简化处理
...
func (m *MQManager) publishRabbitMQ(ctx context.Context, message *Message) error {
	// 这里应该调用RabbitMQ发布消息
	// 简化处理
...
func (m *MQManager) publishKafka(ctx context.Context, message *Message) error {
	// 这里应该调用Kafka发布消息
	// 简化处理
...
func (m *MQManager) processMessages() {
	for message := range m.memoryQueue {
		m.mutex.RLock()
		handler, exists := m.handlers[message.Topic]
		m.mutex.RUnlock()
